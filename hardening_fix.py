...
"""
    ...
    def run_script(self, script_name, scan_only=False):
        ...
        try:
            ...
            return {'status': 'error', 'message': f'Cannot make script executable: {str(e)}'}
    ...
    def generate_report(self):
        ...
        if not self.results:
            print(f"{Colors.YELLOW}[!] No results to report{Colors.ENDC}")
            return
        ...
        try:
            ...
            print(f"{Colors.RED}[ERROR] Failed to generate report: {str(e)}{Colors.ENDC}")
    ...
    def run_all_modules(self, scan_only=False):
        ...
        for module_id, module_info in self.modules.items():
            ...
            if result['status'] == 'success':
                print(f"\n{Colors.GREEN}[✓] Module completed successfully{Colors.ENDC}")
            elif result['status'] == 'warning':
                print(f"\n{Colors.YELLOW}[!] Module completed with warnings{Colors.ENDC}")
            else:
                print(f"\n{Colors.RED}[✗] Module failed: {result.get('message', 'Unknown error')}{Colors.ENDC}")
    ...
    def run_single_module(self, module_id, scan_only=False):
        ...
        if result['status'] == 'error':
            ...
            print(f"\n{Colors.YELLOW}[!] Module completed with warnings{Colors.ENDC}")
            if result.get('stderr'):
                print(f"{Colors.YELLOW}Stderr output:{Colors.ENDC}")
                print(result['stderr'])
        else:
            print(f"\n{Colors.GREEN}[✓] Module completed successfully{Colors.ENDC}")
